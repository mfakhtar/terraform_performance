...
perform_tasks() {
    ...
    local script_path="/Users/mohdfawazakhtar/terraform-cloud/terraform_performance/new_test/count_argument/main.tf"
...
#    cp "$script_path" .
    ...
    plan_time=$( { time terraform plan -refresh=false; } 2>&1 | grep -e real -e user -e sys | awk '{print $2}' | tr '\n' ',' )
    
    # Apply with Terraform without prompting for input
    apply_time=$( { time terraform apply -refresh=false --auto-approve; } 2>&1 | grep -e real -e user -e sys | awk '{print $2}' | tr '\n' ',' )
