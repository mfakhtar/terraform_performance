...
echo "Folder Name,Init Real Time (s),Init User Time (s),Init Sys Time (s),Random Real Time (s),Random User Time (s),Random Sys Time (s),Plan Real Time (s),Plan User Time (s),Plan Sys Time (s),Apply Real Time (s),Apply User Time (s),Apply Sys Time (s),Refresh Real Time (s),Refresh User Time (s),Refresh Sys Time (s)" > terraform_times.csv
...
perform_tasks() {
    ...
    local module_path="/Users/mohdfawazakhtar/terraform-cloud/terraform_performance/new_test/module_usage/null_module"

    # Create directory and change to it
    mkdir -p "$folder_name"
    cd "$folder_name"

    # Copy script from root directory
    cp "$script_path" .
    cp -R "$module_path" .
    echo "count1 = $resource_count" > terraform.tfvars
